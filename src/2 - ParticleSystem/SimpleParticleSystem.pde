...
void update(float dt){
  ...
  for (int i = 0; i < toGen; i++){
    ...
    vel[numParticles] = new Vec2(random(60) + 30,-200 + random(10)); 
    numParticles += 1;
  }
  ...
  if (leftPressed && upPressed) obstacleVel = new Vec2(-obstacleSpeed,-obstacleSpeed);
  else if (leftPressed && downPressed) obstacleVel = new Vec2(-obstacleSpeed,obstacleSpeed);
  else if (leftPressed) obstacleVel = new Vec2(-obstacleSpeed,0);
  
  if (rightPressed && upPressed) obstacleVel = new Vec2(obstacleSpeed,-obstacleSpeed);
  else if (rightPressed && downPressed) obstacleVel = new Vec2(obstacleSpeed,obstacleSpeed);
  else if (rightPressed) obstacleVel = new Vec2(obstacleSpeed,0);
  if(shiftPressed) {
    obstacleVel = obstacleVel.times(2.0);
  }
