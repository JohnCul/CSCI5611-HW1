...
int closestNode(Vec2 point){
  ...
  for(int i = 1; i < numNodes; i++){
    if(point.distanceTo(nodePos[i]) < point.distanceTo(nodePos[closestIndex])){
      closestIndex = i;
    }
  }
  return closestIndex;
...
void generateRandomNodes(Vec2[] circleCenters, float[] circleRadii, Vec2 boxTopLeft, float boxW, float boxH){
  for (int i = 0; i < numNodes; i++){
    ...
    boolean isInRect = pointInBox(boxTopLeft, boxW, boxH, randPos);
    while (isInRect){
      randPos = new Vec2(random(width),random(height));
      isInRect = pointInBox(boxTopLeft, boxW, boxH, randPos);
    }
...
void connectNeighbors(){
  for (int i = 0; i < numNodes; i++){
    ...
    for (int j = 0; j < numNodes; j++){
      ...
      hitInfo rectListCheck = rayBoxIntersect(boxTopLeft, boxW, boxH, nodePos[i], dir, distBetween);
      if (!circleListCheck.hit && !rectListCheck.hit && distBetween < 200){
        neighbors[i].add(j);
      }
    }
  }
