...
void setup(){
  ...
  agentPos[3] = new Vec2(20,20);
  agentPos[4] = new Vec2(520,520);
  goalPos[0] = new Vec2(200,420);
  goalPos[1] = new Vec2(120,120);
  goalPos[2] = new Vec2(220,220);
  goalPos[3] = new Vec2(520,580);
  goalPos[4] = new Vec2(20,380);
...
Vec2 computeAgentForces(int id){
  ...
  Vec2 goalVel = goalPos[id].minus(agentPos[id]).times(k_goal);
  Vec2 goal_force = goalVel.minus(agentVel[id]); 
  acc.add(goal_force);
  for(int i=0; i<agentPos.length; i++){
    if(i != id && goalPos[id].distanceTo(agentPos[id]) > 10){
      ...
      if(ttc > 0){
        ...
        Vec2 dir = A_future.minus(B_future);
        dir.normalize();
        float t_h = 2.0;
        acc.add(dir.times(k_avoid * ((t_h-ttc)/ttc)));
      }
    }
  }
  //if(goalPos[id].distanceTo(agentPos[id]) < 5){
  //  acc = new Vec2(0,0);
  //}
  return acc;
}
...
void draw(){
  ...
  fill(20,200,150);
  for (int i = 0; i < numAgents; i++){
    circle(agentPos[i].x, agentPos[i].y, (agentRad*2) - 10);
  }
}
